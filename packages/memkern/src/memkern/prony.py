"""
Prony / Matrix-Pencil decomposition of exponential sums.

Fits a real or complex time series ``y(τ)`` by a sum of exponentials

        y(τ) ≈ Σₖ αₖ · exp(-βₖ · τ)        (Re βₖ ≥ 0)

This is a **domain-agnostic** routine. It is the same decomposition used
in physics (bath correlation functions), rheology (Maxwell-Wiechert /
generalized Maxwell models), climate science (impulse response
functions), and time-series analysis (exponential smoothing).

Why Matrix Pencil (not classical Prony, not ESPRIT):
  - Original Prony is ill-conditioned on noisy data — it collapses
    all spectral information into a single polynomial root extraction.
  - ESPRIT (rotational invariance) and Matrix Pencil are both SVD-based,
    ...
    limit. Matrix Pencil has slightly simpler code and keeps the full
    SVD spectrum for automatic order selection (``maxent_select_order``,
    ``_auto_select_order``).
  - AAA rational approximation is an alternative reserved for a future
    backend.

See ``maxent_select_order`` for the MaxEnt-based order-selection criterion,
a principled alternative to variance-ratio truncation.
...
def maxent_select_order(
    ...
    *,
    ...
    See ``docs/DOI_REGISTRY.md`` at the suite root — level 3 (atomic).
    Module DOI: pending (to be minted with memkern 0.1.0 PyPI release).
    Suite DOI: 10.5281/zenodo.19486927 (covers citation until the
    module-specific DOI is active).
